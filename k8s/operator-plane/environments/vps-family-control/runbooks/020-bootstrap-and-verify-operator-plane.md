...
- explicit `operator-vault` Traefik TCP passthrough endpoint with `--operator-vault-public-endpoint`
...
JSON is not saved, and the CA private key remains inside OpenBao. The
`--operator-vault-tls` phase is intentionally explicit and is not included in
`--all` yet because it rotates OpenBao runtime TLS and restarts only
`openbao-global-0`.

The `--operator-vault-public-endpoint` phase exposes
`operator-vault.${OPERATOR_DOMAIN}` through Traefik `IngressRouteTCP` on
`websecure` with `HostSNI` and `tls.passthrough=true`. The route forwards to
`openbao-global:8200`; Traefik does not terminate TLS, does not use Let's
Encrypt, does not use an HTTP IngressRoute, and does not reference a Kubernetes
TLS Secret. OpenBao terminates TLS with the Operator CA-issued runtime
certificate, and clients must verify it with
`OPERATOR_PKI_PUBLIC_DIR/operator-ca-bundle.pem`.

The Operator CA bundle is public trust material, not an access restriction.
Network access is restricted by Traefik MiddlewareTCP `ipAllowList` using
`OPERATOR_VAULT_ALLOWED_SOURCE_RANGES`, which should be equal to or stricter
than `OPERATOR_ARTIFACTS_ALLOWED_SOURCE_RANGES`. OpenBao authentication remains
mandatory after the network and TLS checks pass.
...
Do not run destructive wipe behavior or destructive reinstall tests until Operator PKI, operator-vault TLS, operator-vault public endpoint, artifact publication, and final debug points are complete.
...
Preview the explicit `operator-vault` public endpoint phase:
...
Run the explicit `operator-vault` public endpoint phase:
