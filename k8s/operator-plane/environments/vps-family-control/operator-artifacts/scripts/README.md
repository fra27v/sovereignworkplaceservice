...
- `update-operator-artifacts-ip-allowlist.sh`: updates the gitignored environment allowlist, reapplies operator-artifacts, and verifies the deployment.

Use `update-operator-artifacts-ip-allowlist.sh` when the home public IP changes.
The script updates the real local `operator-artifacts.env` file, creates a
