...
ensure_public_operator_pki_path_permissions() {
  local public_dir="$1"
  local public_parent

  public_parent="$(dirname -- "${public_dir}")"
  [[ -d "${public_parent}" ]] || fail "Public Operator PKI parent directory is missing: ${public_parent}"

  chown root:root "${public_parent}" "${public_dir}"
  chmod 0711 "${public_parent}"
...
ensure_public_operator_pki_path_permissions "${OPERATOR_PKI_PUBLIC_DIR}"
