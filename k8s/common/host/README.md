...
This directory contains the reusable Ubuntu host baseline.
...
The scripts currently keep the original `/etc/*family-infra*` drop-in filenames
for SSH, fail2ban, and unattended-upgrades. This avoids an invasive migration
on hosts that already applied the earlier profile while keeping the repository
layout and script entrypoints generic.
